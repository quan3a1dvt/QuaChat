...
function startChatWithFriend(userId) {
  SocketioService.startChatWithFriend(userId);
}
...
async function captureThumbnail(msgId) {
  if (currentRoom.value.messages[msgId].sent == false) {
    setTimeout(() => {
      ...
      canvas.toBlob((blob) => {
        ...
        SocketioService.uploadVideoThumbnail(thumbnailFile, msgId);
      }, "image/jpeg");
    }, 500);
  }
}
  ...
  <div class="WAL position-relative bg-grey-4" :style=style>
    <q-layout view="lHr lpR lFr" class="WAL__layout shadow-3" container>
                      ...
                      <q-scroll-area class="q-mx-sm" style="height: 300px">
                        <template v-for="user in currentRoom.users">
                          <q-item
                            ...
                            "
                            @click="startChatWithFriend(user.id)"
                            v-close-popup
                          >
      ...
      <q-drawer
        ...
        :breakpoint="690"
      >
        <q-toolbar class="bg-grey-1 q-pl-lg" style="height: 65px">
        ...
        <div class="q-px-sm bg-grey-1">
          <div class="q-px-md q-mb-md">
            <q-input
              dense
              class="full-width search-field shadow-1"
              bg-color="white"
              color="teal-6"
            ...
            <q-scroll-area style="height: 750px">
              <q-list>
                <template v-for="roomId of store.sortedRoomsId" :key="roomId">
                  <q-item
                    v-if="store.rooms[roomId].name.includes(searchRoomName)"
                    clickable
                    v-ripple
                  ...
                  >
                    ...
                    {{
                      ((readIdx = room.users[store.loginUser.id].readIdx), null)
                    }}
                    {{ ((unreadMsg = msgsIdArr.length - (readIdx + 1)), null) }}
                    <template v-if="msgsIdArr.length > 0">
                      {{
                        ((lastMsg =
                          room.messages[msgsIdArr[msgsIdArr.length - 1]]),
                        null)
                      }}
                      <q-item-section avatar>
                        <q-avatar size="xl">
                          ...
                          <q-badge
                            floating
                            :color="
                              getRoomStatus(room.id) ? 'green' : 'transparent'
                            "
                            rounded
                          />
                        </q-avatar>
                      ...
                      <q-item-section>
                        <q-item-label lines="1" class="parent">
                          <div
                            class="dynamic-width-child text-subtitle1 text-weight-medium"
                          >
                            {{ room.name }}
                          </div>
                          <q-space />
                          <div
                            :class="{
                              'text-weight-bold text-teal-6': unreadMsg > 0,
                              'fixed-width-child text-weight-light': true,
                            }"
                            style="display: flex; align-items: center"
                          >
                            {{ getShortTimeFormat(lastMsg.createon) }}
                          </div>
                        </q-item-label>
                        <q-item-label
                          class="conversation__summary parent text-caption"
                        >
                          <div class="dynamic-width-child">
                            {{
                              lastMsg.type == MESSAGE_TYPE.TEXT
                                ? `${
                                    lastMsg.from == store.loginUser.id
                                      ? "You"
                                      : store.users[lastMsg.from].displayname
                                  }: ${lastMsg.content}`
                                : lastMsg.type == MESSAGE_TYPE.EVENT
                                ? lastMsg.content
                                : `${
                                    lastMsg.from == store.loginUser.id
                                      ? "You"
                                      : store.users[lastMsg.from].displayname
                                  } sent a ${MESSAGE_TYPE2TEXT[lastMsg.type]}`
                            }}
                          </div>
                          <q-space></q-space>
                          <div v-if="unreadMsg > 0" class="fixed-width-child">
                            <q-badge
                              color="teal-6"
                              rounded
                              :label="unreadMsg"
      ...
      >
        <q-toolbar class="bg-grey-3">
          <q-btn round flat icon="mdi-arrow-right" @click="toggleRightDrawer"></q-btn>
        </q-toolbar>
        <div>
          ...
          <q-card class="full-width"> </q-card>
        </div>
      </q-drawer> -->
      <q-page-container class="bg-grey-2" style="z-index: 2">
        <q-page style="height: 1px">
          ...
          >
            ...
            {{ ((msgsIdArr = Object.keys(currentRoom.messages)), null) }}
            {{ (msgsIdArrLoad = msgsIdArr.slice(currentRoom.loadMsgIdx), null)}}
          ...
          <q-infinite-scroll reverse @load="onLoad">
            ...
            <div
              v-for="messageId of msgsIdArrLoad"
              :key="messageId"
              :id="messageId"
            >
              {{ (message = currentRoom.messages[messageId], null) }}
              {{ ((isSender = message.from == store.loginUser.id), null) }}

              <div
                :style="
                  message.from == store.loginUser.id
              ...
              >
                <q-chat-message
                  unelevated
                  :class="{
                    'remove-triagle': isSender || !message.isStartMsg,
                    'top-triagle top-avatar': true,
                  }"
                  @dblclick="msgMenuOpen = true"
                  :sent="isSender"
                  :bg-color="
                    message.from == store.loginUser.id ? 'green-2' : 'white'
                  "
                >
                  <template v-slot:avatar>
                    <img
                      :style="{
                        visibility: message.isStartMsg ? 'visible' : 'hidden',
                        display:
                          message.from == store.loginUser.id ? 'none' : '',
                      }"
                      class="q-message-avatar q-message-avatar--received"
                      :src="store.users[message.from].avatar"
                    />
                  </template>
                  <div>
                    ...
                    >
                      <span class="text-bold text-green-6">{{
                        message.replyMsg.from == store.loginUser.id
                          ? "You"
                          : store.users[message.replyMsg.from].displayname
                      }}</span>
                    ...
                    <div v-if="message.type == MESSAGE_TYPE.VIDEO">
                      <video
                        ...
                        :id="`video-${message.id}`"
                        crossorigin=""
                        @loadedmetadata="captureThumbnail(message.id)"
                        :src="message.file.src"
                        type="video/mp4"
                      ></video>
                    </div>
                    ...
                    <div
                      :class="{
                        'q-mt-sm': message.type == MESSAGE_TYPE.TEXT,
                        'q-ml-lg': true,
                      }"
                    >
                ...
                <div
                  v-if="message.from == store.loginUser.id"
                  class="q-mb-sm"
                ...
                >
                  <span
                    ...
                    :style="
                      message.sent == false ||
                      msgsIdArr.indexOf(message.id) == msgsIdArr.length - 1
                        ? 'visibility:visible'
                        : 'visibility:hidden'
                    "
                  >
