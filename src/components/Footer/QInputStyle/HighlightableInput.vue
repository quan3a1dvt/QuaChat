...
export default {
  ...
  computed: {
    tag() {
      ...
      const index = this.internalValue.lastIndexOf('@')
      if (index > -1) {
        if (index > 0) {
          if (this.internalValue.charAt(index - 1) != ' ') return null
        }
        const ttag = this.internalValue.substring(index + 1)
  ...
  watch: {
    ...
    htmlOutput() {
      ...
      this.$emit('value-update', (this.$el.textContent))
    }
