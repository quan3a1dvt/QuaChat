...
const useMainStore = defineStore('main', {
  ...
  getters: {
    ...
    mediaMessages: (state) => (
      Object.fromEntries(Object.entries(state.rooms[state.currentRoomId].messages).filter(([k,v]) => (v.type == MESSAGE_TYPE.IMAGE || v.type == MESSAGE_TYPE.VIDEO)))
    ),
    sortedRoomsId: (state)  => {
      ...
      return roomsIdArr.sort(function(a, b){
        ...
        return state.rooms[b].messages[msgsIdArrB[msgsIdArrB.length - 1]].createon - state.rooms[a].messages[msgsIdArrA[msgsIdArrA.length - 1]].createon
      });
    },
  },
  ...
  actions: {
      ...
      if (!this.rooms.hasOwnProperty(room.id)) {
        ...
        msgsIdArr.forEach(function (msgId, index) {
          if (index == 0) room.messages[msgId].isStartMsg = true
            ...
            room.messages[msgId].isStartMsg = isStartOfMsgGroup(currentMsg, preMsg)
          }
        });
    ...
    addMessage(msg) {
      ...
      if (msgsIdArr.length == 0) msg.isStartMsg = true
        ...
        msg.isStartMsg = isStartOfMsgGroup(msg, this.rooms[msg.to].messages[msgsIdArr[msgsIdArr.length - 1]])
      }
    ...
    setEmote(roomId, messageId, userId, emote) {
      this.rooms[roomId].messages[messageId].emotes[{'userId': userId, 'emote': emote}] = true
    }
  },
});
