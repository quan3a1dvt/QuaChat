...
app.get('/register', async (req, res) => {
  ...
  if (user_info == undefined) {
    let id = String(parseInt((await users_login.find().sort({ id: -1 }).limit(1).toArray())[0].id) + 1)
    users_login.insertOne({
      email: req.query.email,
      password: req.query.password,
      id: id
    })
    createUser(id, req.query.username)
...
io.on('connection', async (socket) => {
  ...
  socket.on("sendMessage", async (msg) => {
    if (msg.type == 'file') {
      ...
      msg.file.src = `${ip}:3000/image?roomId=${msg.to}&file_id=${msg.file.id}&file_name=${msg.file.name}`
    }
    else if (msg.type == 'video') {
      msg.file.src = `${ip}:3000/video?roomId=${msg.to}&file_id=${msg.file.id}&file_name=${msg.file.name}`
      msg.file.thumbnail = `${ip}:3000/video/thumbnail?roomId=${msg.to}&file_id=${msg.file.id}&file_name=${msg.file.name}`
    }
